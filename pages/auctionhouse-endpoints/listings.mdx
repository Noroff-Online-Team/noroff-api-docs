...
The list of listings can be sorted with the `sort` & `sortOrder` flags:
...
> Please note that listings `media` must be an array of fully formed URLs that links to a live and publicly accessible images. The API will check the provided URLs and if any cannot be accessed publicly you will receive a 400 error response.
